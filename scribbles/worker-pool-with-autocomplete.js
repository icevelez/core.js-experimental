 ...
 * @typedef {(...args: Parameters<T>) => Promise<Awaited<ReturnType<T>>>} WorkerFunction
 */

 /**
  * @template {any} T
  * @typedef {{ register< K extends string, F>(name: K, fn: F): WorkerPool<T & { [P in K]: F }>; worker: { [K in keyof T]: T[K] }; }} WorkerPool
*/

export const worker_pool = Object.freeze({
    /**
     * @template {any} T
     * @returns {WorkerPool<T>}
     */
    create_worker: () => ({
        register(func_name, fn) {
            if (!this.function_cache) this.function_cache = new Map();
            this.function_cache.set(func_name, new_func(fn.toString()));

            const function_cache = this.function_cache;

            return {
                register: this.register,
                worker : new Proxy(Object.create(null), {
                    get(_, key) {
