...
class Worklet {
     ...
     * @param {F} fn
     *
     * @returns {Worklet<T & { [P in K]: (...args: Parameters<F>) => Promise<Awaited<ReturnType<F>>> } >}
     */
    register = (fn_name, fn) => {
    ...
    run = (() => {
        ...
        return new Proxy(Object.create(null), {
            get(_, key) {
                ...
                if (!func) console.error(`[worker-pool]: (run) function "${String(key)}" is not registered, returning undefined`);
                return func;
            },
        })
    })();
...
/** @type {() => Worklet<any>} */
export const create_worker = () => new Worklet();
